...
Train a seq2seq model for a dialog agent

based on the example in Keras: https://github.com/keras-team/keras/blob/master/examples/lstm_seq2seq.py
basic character-level sequence-to-sequence model
Python 3
...
limit = 10000  # maximum number of samples to train on.
'''
...
data_path = 'data/generated_train.txt'
...
class Seq2Seq():
    ...
    def __init__(self):
        '''
        '''
        self.max_encoder_seq_length = 62
        self.max_decoder_seq_length = 32
        self.latent_dim = 256
    ...
    def restore_model(self, path):
        ...
        # load token indices
        with open(path_input_index, 'rb') as handle:
            self.input_token_index = pickle.load(handle)
            self.num_encoder_tokens = len(self.input_token_index)
        with open(path_target_index, 'rb') as handle:
    ...
    def preprocess(self, data_path):
        ...
        return (input_texts, encoder_input_data, decoder_input_data, decoder_target_data)
    ...
    def train(self, encoder_input_data, decoder_input_data, decoder_target_data):
        ...
        # save indices
        with open(path_input_index, 'wb') as handle:
    ...
    def test(self, input_texts, encoder_input_data):
