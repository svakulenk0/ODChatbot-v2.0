...
class ODEvaluator(GenericEvaluator):
    def __init__(self, result_path):
        super().__init__(result_path)
        self.entities = []
        self.entity_dict = {}
    ...
    def run_metrics(self):
        raw_json = open('./data/OD/dialogues.json')
        raw_entities = open('./data/OD/entities.json')
        raw_data = json.loads(raw_json.read().lower())
        raw_entities = json.loads(raw_entities.read().lower())
        self.get_entities(raw_entities)
        data = self.read_result_data()
    ...
    def _extract_request(self, z):
        z = z.split()
        # requestable
        return set(z).intersection(['dataset_name', 'dataset_link'])
